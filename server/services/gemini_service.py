...
class GeminiService:
    def __init__(self) -> None:
        ...
        self.model_workhorse = genai.GenerativeModel('gemini-1.5-flash')
        
        # Tactical Buffer: 10 RPM / 20 RPD - for tactical insights (every 20s)
        self.model_tactical = genai.GenerativeModel('gemini-2.0-flash-lite')
        
        # Chat: 5 RPM / 20 RPD - for user-triggered Q&A
        self.model_chat = genai.GenerativeModel('gemini-1.5-flash')
        
        # Legacy fallback for backward compatibility
        self.model = self.model_workhorse

    def generate_insight(self, match_context: Dict[str, Any], insight_type: str) -> Dict[str, Any]:
        ...
        try:
            # Use workhorse model (15 RPM / 500 RPD)
            response = self.model_workhorse.generate_content(prompt)
            text = response.text or ''
            return self._parse_insight_response(text, insight_type)
        except Exception:
            return self._fallback_insight(insight_type)
    ...
    def _build_insight_prompt(self, match_context: Dict[str, Any], insight_type: str) -> str:
        ...
        if batter.get('name'):
            context_info.append(f"Active Batter: {batter.get('name')} ({batter.get('role', 'Batter')})")
        if bowler.get('name'):
            context_info.append(f"Active Bowler: {bowler.get('name')} ({bowler.get('role', 'Bowler')})")
    ...
    def generate_win_probability_history(self, match_id: str = "current") -> Dict[str, Any]:
    ...
    def generate_tactical_insights(self, match_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_completed = any(keyword in status.lower() for keyword in ['won', 'win', 'lost', 'complete'])
                    ...
                    'insights': [
                        {
                            ...
                            'label': parsed['momentumShift'].get('label', 'Momentum Shift' if not is_completed else 'Match Summary'),
                        ...
                        },
                        {
                            ...
                            'type': 'tactical',
                            'label': parsed['tacticalRead'].get('label', 'Tactical Read' if not is_completed else 'Tactical Analysis'),
    ...
    def _fallback_tactical_insights(self, match_context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    def answer_tactical_question(self, question: str, match_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Uses Gemini 1.5 Flash (5 RPM / 20 RPD) - User-triggered Q&A"""
        ...
        try:
            # Use chat model (5 RPM / 20 RPD) - User-triggered
    ...
    def generate_batched_insights(self, match_context: Dict[str, Any]) -> Dict[str, Any]:
        """Generate all 3 insight types in a SINGLE API call to save quota
        Uses Gemini 1.5 Flash (15 RPM / 500 RPD) - Workhorse model"""
        if not self.model_workhorse:
        ...
        try:
            # Use workhorse model (15 RPM / 500 RPD) - Single call for all 3 insights
            response = self.model_workhorse.generate_content(prompt)
            text = response.text or ''
            parsed = self._extract_json(text)
            
            if parsed and all(key in parsed for key in ['tactical', 'momentum', 'prediction']):
                timestamp = int(time.time() * 1000)
                insights = []
    ...
    async def generate_all_insights_parallel(self, match_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            general_insights = results[0] if not isinstance(results[0], Exception) else self._fallback_batched_insights()
            tactical_insights = results[1] if not isinstance(results[1], Exception) else self._fallback_tactical_insights(match_context)
